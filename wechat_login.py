...
PNG_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "wechat-login-qr.png")
...
def main():
    # 1. 取二维码
    qr = get_json(BASE + "ilink/bot/get_bot_qrcode?bot_type=" + BOT_TYPE, timeout=20)
    content = qr.get("qrcode_img_content")
    token = qr.get("qrcode")
    if not content or not token:
        print("QR_FETCH_FAIL " + json.dumps(qr)[:200], flush=True); return 5
    ...
    while time.time() < deadline:
        try:
            ...
            print("POLL_RETRY " + str(e)[:120], flush=True); time.sleep(1.5); continue
        s = st.get("status")
        if s == "confirmed":
            if not st.get("ilink_bot_id") or not st.get("bot_token"):
                print("LOGIN_FAIL missing_creds", flush=True); return 2
            account = {
                "token": st["bot_token"],
                "baseUrl": st.get("baseurl") or BASE,
                "accountId": st["ilink_bot_id"],
                "userId": st.get("ilink_user_id"),
                "savedAt": datetime.datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.000Z"),
            }
            os.makedirs(DATA_DIR, exist_ok=True)
            with open(CRED_FILE, "w", encoding="utf-8") as f:
                json.dump(account, f, ensure_ascii=False, indent=2)
            # 不打印 token；只打印非敏感标识
            print("LOGIN_CONFIRMED account=%s user=%s" % (account["accountId"], account["userId"]), flush=True)
            return 0
        if s == "expired":
            print("QR_EXPIRED", flush=True); return 3
        if s == "scaned" and not scanned:
            scanned = True; print("QR_SCANNED_waiting_confirm", flush=True)
        time.sleep(1.5)
    print("TIMEOUT", flush=True); return 4
