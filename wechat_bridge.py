...
# 容错: 模型可能把结尾 ]] 写成中文】或漏写(实测踩过) -> 开头也认全角、结尾认 ]]/】】/混排/直到文本末尾
BGTASK_RE = re.compile(r"[\[【]{2}\s*BGTASK[:：]\s*(.+?)\s*(?:[\]】]{2}|$)", re.S)
...
BRIDGE_PROMPT = (
    ...
    "指令的方括号必须用半角 [[ 和 ]], 尤其结尾必须是半角 ]], 不要写成中文】。"
    "注意后台会话看不到你们的聊天记录, 所以 BGTASK 描述必须自包含: 把需求、数量、路径等聊天里已确认的上下文全部写进去。"
    "正文里简短告诉主人任务已转后台即可。简单问题照常直接回答, 不要滥用后台。"
    "【严禁空头承诺】你回复完这条消息, 你的进程就立刻结束了, 没有任何东西会替你'稍后继续'——"
    "所以绝不允许在回复里写'正在跑/稍后继续/跑完我会…'这类话, 也不要自己启动转写、下载等长进程再把结果留给'以后的自己'。"
    "只有两个选择: 要么当场彻底做完再回复, 要么整个任务(含你已经做到一半的部分和已产出文件的路径)写成 BGTASK 转后台。"
)
...
def split_bg_directives(reply):
    ...
    def repl(m):
        # 结尾括号可能是全角/单只/漏写, 捕获组里残留的收尾括号一并剥掉
        d = m.group(1).strip().rstrip("]】").strip()
        if d:
            descs.append(d)
        return ""
